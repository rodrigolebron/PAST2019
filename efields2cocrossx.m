function [u_co_2,u_cross_2,u_co_3,u_cross_3,u_co_2_dB,u_cross_2_dB,u_co_3_dB,u_cross_3_dB] = efields2cocrossx(THETA,PHI,Etheta,Ephi)

%efields2cocrossy Calculates Ludwig's Definition of Co- and 
%Cross-Polarization for an antenna that is polarized along the X-axis.
%
...
u_co_2 = (cos(THETA).*cos(PHI).*Etheta-sin(PHI).*Ephi)./(sqrt(1-(sin(THETA).*cos(PHI)).^2));
u_cross_2 = (sin(PHI).*Etheta+cos(THETA).*cos(PHI).*Ephi)./(sqrt(1-(sin(THETA).*cos(PHI)).^2));
...
u_co_3 = cos(PHI).*Etheta-sin(PHI).*Ephi;
u_cross_3 = sin(PHI).*Etheta+cos(PHI).*Ephi;
